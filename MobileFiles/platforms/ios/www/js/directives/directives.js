    ...
    }).directive('otpCheck', function ( AuthServices, $q) {
        return {
            restrict: 'A',
            require: ['ngModel', '^form'],
            scope: true,
            link: function (scope, element, attributes, ctrls) {
                ...
                var ngModel = ctrls[0];
                var form = ctrls[1];
                ngModel.$asyncValidators.checkOtp = function (modelValue, viewValue) {
                    return $q(function (resolve, reject) {
                        AuthServices.isOTPValid(modelValue).then(function () {
                            resolve();
